...
class FFT {
    ...
    func calculate(_ _values: [Double], fps: Double) -> ([Double], [Double], Double){
        ...
        values.withUnsafeMutableBytes {
            valuesAsComplex = $0.baseAddress?.bindMemory(to: DSPDoubleComplex.self, capacity: localValues.count)
        }
        ...
        // Reduce array to non 0 elements
        var retSpectrum = [Double]()
        var retSeries = [Double]()
        for index in bandPassFilter.1 ..< (bandPassFilter.2+1) {
            retSpectrum.append( filteredSpectrum[index])
            retSeries.append( freqs[index])

        }
//        for (index, element) in bandPassFilter.enumerated() {
