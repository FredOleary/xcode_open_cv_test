//
//  IcaFftDataViewController.swift
//  mac_min2
//
...
class IcaFftViewController: UIViewController, ChartReadyDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        print("IcaFftViewController - viewDidLoad")
    ...
    func updateGraph(){
        if let timeSeries = heartRateData?.ICAFFTRedFrequency {
            if( timeSeries.count > 0){
                ...
                let redFreq = NSString(format: "Red BPM %.2f", (heartRateData!.ICAheartRateRedFrequency! * 60))
                let greenFreq = NSString(format: "Green BPM %.2f", (heartRateData!.ICAheartRateGreenFrequency! * 60))
                let blueFreq = NSString(format: "Blue BPM %.2f", (heartRateData!.ICAheartRateBlueFrequency! * 60))
                addBar(data, heartRateData!.ICAFFTRedAmplitude!, timeSeries, color:[NSUIColor.red], redFreq as String)
                addBar(data, heartRateData!.ICAFFTGreenAmplitude!, timeSeries, color:[NSUIColor.green], greenFreq as String)
                addBar(data, heartRateData!.ICAFFTBlueAmplitude!, timeSeries, color:[NSUIColor.blue], blueFreq as String)
                ...
                barChart.data = data
                barChart.chartDescription?.text = "ICA - FFT"
